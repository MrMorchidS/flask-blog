...
def register():
    form = RegisterForm()
    # If the user already login redirect him to  get_all_posts.
    if current_user.is_authenticated:
        return render_template(url_for("get_all_posts"))
    
    # Check if the form data is valid and has been submitted.
    if form.validate_on_submit():
        ...
        if User.query.filter_by(username=form.username.data).first() is None:
            ...
            if User.query.filter_by(email=form.email.data).first() is None:
                ...
                flash("The email you provided is already registered. Please log in or use a different email address.", "error")
                return redirect(url_for("login"))
        else:
            flash("The username is already taken. Please choose a different username.", "error")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You should login before adding comments.", "error")
            return redirect(url_for("login"))
